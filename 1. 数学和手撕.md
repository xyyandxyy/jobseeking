...
def calculate_auc(y_labels, y_scores):
    samples = list(zip(y_scores, y_labels)) # 打包一下所有样本的logit及其标签
    rank = [(values2, values1) for values1, values2 in sorted(samples, key=lambda x:x[0])] # 按照概率排序
    pos_rank = [i+1 for i in range(len(rank)) if rank[i][0] == 1] # 挑出正样本的rank
