...
- **计算单元（CUDA Core/Tensor Core）只能访问SRAM**（寄存器、共享内存）
- ❌ **HBM不能直接参与计算**
...
图片经过vit (使用14x14的patch), vit每一 个block输出都是[5,577,1024], 第一个是5张图片, 其中1张原图, 4张patches
...
4. 最后由`Qwen2_5_VLPatchMerger`进行pixel shuffling (patch merge), 来减少token 数量, 就是空间维度缩小一倍(长和宽都缩小一倍, 面积缩小到1/4). 
   具体的就是输入reshape成`(768/4, 1280*4)`, 即`(192, 5120)`,然后用一个MLP去降维, 最终变成`(192,3584)`. 最终就是有192个token.
...
* min+clip函数的目的：clip就是防止策略更新过大的作用, 同时min的话使clip的裁剪变成非对称, 在A>0的时候移除托底, 能狠狠更新(例如epsilon=0,2的时候, 避免is_ratio=0.5 和 is_ratio=0.8对应的loss是相同的, 因为明明新策略导致了当前好结果出现的概率变低了)

  <img src="pic/image-20251110174503652.png" alt="image-20251110174503652" style="zoom:50%;" />

  （1）对偏离rollout过远的采样的梯度进行裁剪，避免它们继续偏离rollout模型。(避免优化太过了)

  （2）对重要性采样的范围进行约束，避免重要性采样引入过大的估计值方差 (避免太不像rollout了)

复习: https://hugging-face.cn/blog/NormalUhr/grpo-to-dapo-and-gspo
...
GRPO 常常因为不合理的截断范围、冗余采样以及长序列中的梯度稀释等问题，浪费了大量的学习信号

* Clip-Higher 解决了“好Token过早被限制”的问题
* Dynamic Sampling 解决了rollout时缺乏采样多样性的情况
* [**在计算梯度时，对所有样本生成的总词元数进行平均**] 修复了 GRPO 中每个词元的**梯度权重**随着采样回答长度增加而减小的问题
* 过长奖励调整: 使用一种**软惩罚**机制来调整过长回答的奖励
...
* $\frac{\pi_\theta\left(y_i \mid x\right)}{\pi_{\theta_{o l d}}\left(y_i \mid x\right)}
  $是一个句子的概率, 是所有词元概率的连乘积. 对于旧模型生成的这个回答，我们的新模型是更「喜欢」它了（概率变高），还是更「讨厌」它了（概率变低）。这才是对重要性采样的正确应用。
